...
public class Keybord_Typing_Spiling_expebnd extends Dictionary {

    protected static final int  TYPINGSPLING = 32;
    private NodeArray nodeary;
    private int[][] intvalue;
    private Context typingcont;
    private char[] typingname = new char[TYPINGSPLING];
    private int typingsplingid;
    private int intmaxvalue;
    private int splingsize;
    private int[] splingtypingword;
    private StringBuilder buliderstring = new StringBuilder(TYPINGSPLING);
    private static final char SLASE = '\'';
    private boolean bollenrlode;
    private boolean addeddecsonary;
    private Object addtingobj = new Object();
    ...
    Keybord_Typing_Spiling_expebnd(Context mcont, int intid) {
        typingcont = mcont;
        clearDictionary();
        intvalue = new int[TYPINGSPLING][];
        typingsplingid = intid;
    }

    public void loadDictionary() {
        synchronized (addtingobj) {
    ...
    public void setRequiresReload(boolean synce) {
        synchronized (addtingobj) {
            bollenrlode = synce;
        }
    }
    ...
    public void addWord(String word, int frequency) {
        addWordRec(nodeary, word, 0, frequency, null);
    }

    private void addWordRec(NodeArray children, final String word, final int depth, final int frequency, Node parentNode) {

        final int msplingsize = word.length();
        final char spling = word.charAt(depth);
        final int sizechild = children.length;
        Node typevalue = null;
        boolean found = false;
        for (int i = 0; i < sizechild; i++) {
            typevalue = children.data[i];
            if (typevalue.code == spling) {
                found = true;
                break;
            }
        }
        if (!found) {
            typevalue = new Node();
            typevalue.code = spling;
            typevalue.parent = parentNode;
            children.add(typevalue);
        }
        if (msplingsize == depth + 1) {
            typevalue.terminal = true;
            typevalue.frequency = Math.max(frequency, typevalue.frequency);
            if (typevalue.frequency > 255) typevalue.frequency = 255;
            return;
        }
        if (typevalue.children == null) {
            typevalue.children = new NodeArray();
        }
        addWordRec(typevalue.children, word, depth + 1, frequency, typevalue);
    }
    ...
    public void getWords(final Keyword_Set_Compres codes, final WordCallback callback, int[] nextLettersFrequencies) {
        ...
        synchronized (addtingobj) {
            if (bollenrlode) startDictionaryLoadingTaskLocked();
            if (addeddecsonary) return;
        }

        splingsize = codes.size();
        splingtypingword = nextLettersFrequencies;
        if (intvalue.length < splingsize) intvalue = new int[splingsize][];
        for (int i = 0; i < splingsize; i++) {
            intvalue[i] = codes.getCodesAt(i);
        }
        intmaxvalue = splingsize * 3;
        getWordsRec(nodeary, codes, typingname, 0, false, 1, 0, -1, callback);
        for (int i = 0; i < splingsize; i++) {
            getWordsRec(nodeary, codes, typingname, 0, false, 1, 0, i, callback);
        }
    }

    @Override
    public synchronized boolean isValidWord(CharSequence seqnce) {
        synchronized (addtingobj) {
            if (bollenrlode) startDictionaryLoadingTaskLocked();
            if (addeddecsonary) return false;
        }
        final int freq = getWordFrequency(seqnce);
        return freq > -1;
    }

    public int getWordFrequency(CharSequence charecter) {
        Node serch = searchNode(nodeary, charecter, 0, charecter.length());
        return (serch == null) ? -1 : serch.frequency;
    }

    protected void getWordsRec(NodeArray roots, final Keyword_Set_Compres codes, final char[] word, final int depth, boolean completion, int snr, int inputIndex, int skipPos, WordCallback callback) {
        final int numvalue = roots.length;
        final int codeSize = splingsize;
        if (depth > intmaxvalue) {
            return;
        }
        ...
        if (codeSize <= inputIndex) {
            ...
            currentChars = intvalue[inputIndex];
        }

        for (int i = 0; i < numvalue; i++) {
            final Node serch = roots.data[i];
            final char mcode = serch.code;
            final char lowerC = toLowerCase(mcode);
            final boolean terminal = serch.terminal;
            final NodeArray nodary = serch.children;
            final int freq = serch.frequency;
            if (completion) {
                word[depth] = mcode;
                if (terminal) {
                    if (!callback.addWord(word, 0, depth + 1, freq * snr, typingsplingid, DataType.UNIGRAM)) {
                        return;
                    }
                    if (splingtypingword != null && depth >= inputIndex && skipPos < 0 && splingtypingword.length > word[inputIndex]) {
                        splingtypingword[word[inputIndex]]++;
                    }
                }
                if (nodary != null) {
                    getWordsRec(nodary, codes, word, depth + 1, completion, snr, inputIndex, skipPos, callback);
                }
            } else if ((mcode == SLASE && currentChars[0] != SLASE) || depth == skipPos) {
                word[depth] = mcode;
                if (nodary != null) {
                    getWordsRec(nodary, codes, word, depth + 1, completion, snr, inputIndex, skipPos, callback);
                }
                ...
                for (int j = 0; j < alternativesSize; j++) {
                    ...
                    if (currentChar == lowerC || currentChar == mcode) {
                        word[depth] = mcode;

                        if (codeSize == inputIndex + 1) {
                            if (terminal) {
                                if (inculde_cuntry || !same(word, depth + 1, codes.getTypedWord())) {
                                    int finalFreq = freq * snr * addedAttenuation;
                                    if (skipPos < 0) finalFreq *= all_country;
                                    callback.addWord(word, 0, depth + 1, finalFreq, typingsplingid,
                                            DataType.UNIGRAM);
                                }
                            }
                            if (nodary != null) {
                                getWordsRec(nodary, codes, word, depth + 1, true, snr * addedAttenuation, inputIndex + 1, skipPos, callback);
                            }
                        } else if (nodary != null) {
                            getWordsRec(nodary, codes, word, depth + 1, false, snr * addedAttenuation, inputIndex + 1, skipPos, callback);
                        }
                    }
                }
            }
    ...
    protected int setBigram(String mspling, String nspling, int frequency) {
        return addOrSetBigram(mspling, nspling, frequency, false);
    }

    protected int addBigram(String mspling, String nspling, int frequency) {
        return addOrSetBigram(mspling, nspling, frequency, true);
    }

    private int addOrSetBigram(String mspling, String nspling, int frequency, boolean addFrequency) {
        Node aspling = searchWord(nodeary, mspling, 0, null);
        Node bspling = searchWord(nodeary, nspling, 0, null);
        LinkedList<NextWord> gramer = aspling.ngrams;
        if (gramer == null || gramer.size() == 0) {
            aspling.ngrams = new LinkedList<NextWord>();
            gramer = aspling.ngrams;
        } else {
            for (NextWord after : gramer) {
                if (after.word == bspling) {
                    if (addFrequency) {
        ...
        NextWord afterspling = new NextWord(bspling, frequency);
        aspling.ngrams.add(afterspling);
        return frequency;
    }

    private Node searchWord(NodeArray children, String word, int depth, Node parentNode) {
        final int splingsize = word.length();
        final char splingcode = word.charAt(depth);
        final int sizechild = children.length;
        Node serch = null;
        boolean found = false;
        for (int i = 0; i < sizechild; i++) {
            serch = children.data[i];
            if (serch.code == splingcode) {
                found = true;
                break;
            }
        }
        if (!found) {
            serch = new Node();
            serch.code = splingcode;
            serch.parent = parentNode;
            children.add(serch);
        }
        if (splingsize == depth + 1) {
            serch.terminal = true;
            return serch;
        }
        if (serch.children == null) {
            serch.children = new NodeArray();
        }
        return searchWord(serch.children, word, depth + 1, serch);
    }

    boolean reloadDictionaryIfRequired() {
        synchronized (addtingobj) {
            if (bollenrlode) startDictionaryLoadingTaskLocked();
            return addeddecsonary;
        }
    }

    private void runReverseLookUp(final CharSequence previousWord, final WordCallback callback) {
        Node beforespling = searchNode(nodeary, previousWord, 0, previousWord.length());
        if (beforespling != null && beforespling.ngrams != null) {
            reverseLookUp(beforespling.ngrams, callback);
        }
    }
    ...
    private void reverseLookUp(LinkedList<NextWord> terminalNodes, final WordCallback callback) {
        Node serch;
        int intfecvancy;
        for (NextWord nextWord : terminalNodes) {
            serch = nextWord.word;
            intfecvancy = nextWord.frequency;
            // TODO Not the best way to limit suggestion threshold
            if (intfecvancy >= User_keybord_Spiling_biigraam.REVIEW_TYPE) {
                buliderstring.setLength(0);
                do {
                    buliderstring.insert(0, serch.code);
                    serch = serch.parent;
                } while(serch != null);

                // TODO better way to feed char array?
                callback.addWord(buliderstring.toString().toCharArray(), 0, buliderstring.length(), intfecvancy, typingsplingid, DataType.BIGRAM);
            }
        }
    }

    private Node searchNode(final NodeArray children, final CharSequence word, final int offset, final int length) {
        // TODO Consider combining with addWordRec
        final int numvalue = children.length;
        char splingvalue = word.charAt(offset);
        for (int j = 0; j < numvalue; j++) {
            final Node node = children.data[j];
            if (node.code == splingvalue) {
                if (offset == length - 1) {
                    if (node.terminal) {
                        return node;
                    }
    ...
    private class LoadDictionaryTask extends AsyncTask<Void, Void, Void> {
        ...
        protected Void doInBackground(Void... v) {
            loadDictionaryAsync();
            synchronized (addtingobj) {
                addeddecsonary = false;
            }
            return null;
        }
    }

    static char toLowerCase(char name) {
        if (name < BASE_CHARS.length) {
            name = BASE_CHARS[name];
        }
        if (name >= 'A' && name <= 'Z') {
            name = (char) (name | 32);
        } else if (name > 127) {
    ...
    static final char BASE_CHARS[] = {
        ...
        0x004f, 0x0055, 0x0055, 0x0055, 0x0055, 0x0059, 0x00de, 0x0073,
